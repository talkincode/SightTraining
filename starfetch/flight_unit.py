...
class FlightUnit(pygame.sprite.Sprite):
    ...
    def __init__(self, type: str, config_data, sound_channel=None):
        ...
        self.direction = pygame.Vector2(random.randint(3, 9),random.randint(3, 9))  
        # 被消灭的时间
        self.kill_time = None
        self.fire_delay = 0
    ...
    def update(self):
        ...
        if self.kill_time:
            ...
            if self.rect.left < 0:
                self.direction.x = abs(random.randint(3, 9))  # 反转x方向的速度

            if self.rect.right > DISPLAY_WIDTH:
                self.direction.x = -abs(random.randint(3, 9))  # 反转x方向的速度

            if self.rect.top < 0:
                self.direction.y = abs(random.randint(1, 4))  # 反转y方向的速度

            if self.rect.bottom > DISPLAY_HEIGHT // 2:
                self.direction.y = -abs(random.randint(2, 4))  # 反转y方向的速度
    ...
    def dodge_fighter(self, target: pygame.sprite.Sprite):
        ...
        if target.rect.centerx < self.rect.centerx:
            # 目标在左侧，尝试向右移动
            self.direction.x = abs(self.direction.x)  # 确保速度为正值，向右移动
        else:
            # 目标在右侧，尝试向左移动
            self.direction.x = -abs(self.direction.x)  # 确保速度为负值，向左移动

        # 确定躲闪方向：目标在上方还是下方
        if target.rect.centery < self.rect.centery:
            # 目标在上方，尝试向下移动
            self.direction.y = abs(self.direction.y)  # 确保速度为正值，向下移动
        else:
            # 目标在下方，尝试向上移动
            self.direction.y = -abs(self.direction.y)  # 确保速度为负值，向上移动
        ...
        if self.rect.x <= 0 or self.rect.right >= DISPLAY_WIDTH:
            self.direction.x = -self.direction.x * 1.5  # 反转方向并加速
        if self.rect.y <= 0 or self.rect.bottom >= DISPLAY_HEIGHT // 2:
            self.direction.y = -self.direction.y * 1.5  # 反转方向并加速
