    ...
    def reset(self):
        ...
        if _sequence:
            for img in res_manager.load_image_sequence(_sequence):
                ...
                scaled_image = pygame.transform.scale(img, (DISPLAY_WIDTH, new_height))
                self.image_sequence.append(scaled_image)

                self.rect_sequence.append(scaled_image.get_rect())
        ...
        # 设置图片的位置
        for _rect in self.rect_sequence + [self.rect]:
            _rect.x = 0
            _rect.y = (
                (DISPLAY_HEIGHT - _rect.height) // 2
                if _rect.height < DISPLAY_HEIGHT
                else 0
            )
